...
public class NewMap implements Serializable {
	...
	/**
	 * Convert the given {@link NewMap} object to XML and write it to the given
	 * outputstream.
	 * 
	 * @param map
	 *            the map to convert
	 ...
	 * @throws JAXBException
	 *             if we fail to convert the map object
	 */
	 ...
	 *             if zone already in the map
	 */
	public void addZone(Zone zone) {
		if (zones.contains(zone)) {
			throw new IllegalArgumentException("zone already in the map "
					+ zone);
		}
		zones.add(zone);
	}
	...
	public int hashCode() {
		try {
			...
			LOGGER.info(
					"Failed to create a stable hash code, reverting back to java jvm unstable.",
					e);
			final int prime = 31;
			int result = 1;
			...
			result = prime * result
					+ ((randomBlocks == null) ? 0 : randomBlocks.hashCode());
			result = prime
					* result
					+ ((randomSequence == null) ? 0 : randomSequence.hashCode());
			result = prime * result
