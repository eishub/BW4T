...
class MenuOptionOpen extends AbstractMenuOption {
	 ...
	 *            The model.
	 */
	public MenuOptionOpen(final MenuBar view, final ScenarioEditorController mainView, BW4TClientConfig model) {
	...
	/**
	 * Shows an open dialogue and proceeds with loading the file chosen into the
	 * GUI if the user confirmed to load in their chosen file.
	 * 
	 * @param configPanel
	 *            The configuration panel of the GUI.
	 * @param entityPanel
	 *            The entity panel of the GUI.
	 */
	private void openFile(ConfigurationPanel configPanel, EntityPanel entityPanel) {
	 ...
	 *            The entity panel of the GUI.
	 * @param file
	 *            The XML file to load into the GUI.
	 */
	private void parseXMLFileIntoGUI(ConfigurationPanel configPanel, EntityPanel entityPanel, File file) {
		try {
	...
	/**
	 * Fills the bot panel with bots that are in the configuration to load in.
	 * 
	 * @param entityPanel
	 *            The entity panel.
	 * @param configuration
	 *            The configuration to load in.
	 */
	...
	/**
	 * Fills the entity panel with e-partners that are in the configuration to
	 * load in.
	 * 
	 * @param entityPanel
	 *            The entity panel.
	 ...
	 *            The configuration to load in.
	 */
	private void fillPanelWithEPartners(EntityPanel entityPanel, BW4TClientConfig configuration) {
		...
		for (int i = 0; i < epartnerRows; i++) {
			...
			String epartnerAmount = Integer.toString(configuration.getEpartner(i).getEpartnerAmount());
