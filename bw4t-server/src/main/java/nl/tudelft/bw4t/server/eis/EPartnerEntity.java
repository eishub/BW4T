...
public class EPartnerEntity implements EntityInterface {
	...
	@Override
	public void connect() {
		this.spawnLocation = new Point2D.Double(this.ourEPartner.getLocation().getX(),
				this.ourEPartner.getLocation().getY());
	}
	...
	/**
	 * This function should be called before perception cycle is started, so that we
	 * can lock the relevant data from the environment.
	 */
	@Override
	public void initializePerceptionCycle() {
		this.ourEPartnerLocation = new Point2D.Double(this.ourEPartner.getLocation().getX(),
				this.ourEPartner.getLocation().getY());
		this.ourEPartnerRoom = RoomLocator.getRoomAt(this.ourEPartnerLocation.getX(), this.ourEPartnerLocation.getY());
	}
	...
	public List<String> getFunctionalities() {
		return this.ourEPartner.getTypeList();
	}
	...
	public long heldBy() throws PerceiveException {
		if (this.ourEPartner.getTypeList().contains(ViewEPartner.FORGET_ME_NOT)
				&& this.ourEPartner.getHolder() != null) {
			return this.ourEPartner.getHolder().getId();
		}
		return -1;
	}

	/**
	 * Percept if the epartner is taken, so other bots won't pick it up if a bot has
	 * forgot it.
	 *
	...
	public long isTaken() throws PerceiveException {
		if (this.ourEPartner.getTypeList().contains(ViewEPartner.FORGET_ME_NOT)
				&& this.ourEPartner.getHolder() != null) {
			return this.ourEPartner.getId();
		}
		return -1;
	}

	/**
	 * Percept if the epartner is left behind, so the epartner knows when to message
	 * the holder.
	 *
	...
	public Point2D getLocation() {
		return new Point2D.Double(this.spawnLocation.getX(), this.spawnLocation.getY());
	}
	...
	public String getRoom() {
		...
		if (this.ourEPartnerRoom == null) {
			return null;
		}
		return this.ourEPartnerRoom.getName();
	}
	...
	public List<String> getRooms() throws PerceiveException {
		...
		if (this.ourEPartner.getTypeList().contains(ViewEPartner.GPS) && this.ourEPartner.hasContext()) {
			for (Object o : this.ourEPartner.getContext().getObjects(Zone.class)) {
				Zone zone = (Zone) o;
	...
	public List<ArrayList<String>> getMessages() {
		List<ArrayList<String>> msg = this.messages;
		this.messages = new LinkedList<>();
		return msg;
	}
	...
	public void sendMessage(String receiver, String message) throws ActException {
		...
		try {
			parameters[0] = Translator.getInstance().translate2Parameter(this.ourEPartner.getName())[0];
	...
	public void receiveMessage(String sender, String message) {
		...
		this.messages.add(messageArray);
	}
}