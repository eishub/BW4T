...
public class RobotEntity implements EntityInterface {
	...
	/**
	 * If false, the "position" percept always return null
	 */
	private boolean reportPositions;
	 ...
	 *            controlling in EIS.
	 * @param reportPositions
	 *            true if this entity should report locations of objects it sees: 
	 *            the "position" percept. If false, those percepts always return 
	 *            null. This is a performance option to reduce percepts generated by 
	 *            bots connected to GOAL. These percepts are needed to let humanbots 
	 *            function properly. If in doubt, set it to true.
	 */
	public RobotEntity(IRobot robot, boolean reportPositions) {
		this.ourRobot = robot;
	...
	@Override
	public void connect() {
		this.spawnLocation = new Point2D.Double(this.ourRobot.getLocation().getX(), this.ourRobot.getLocation().getY());
		this.ourRobot.connect();
	}
	...
	public void disconnect() {
		if (this.ourRobot.isConnected()) {
			// reset before disconnect: reset moves bot to init position.
			reset();
			this.ourRobot.disconnect();
			this.ourRobot.removeFromContext();
		}
	}
	...
	public List<ObjectInformation> getLocations() {
		...
		if (!this.reportPositions || !this.ourRobot.hasContext()) {
			return objects;
		}

		// Add the dropzone
		DropZone dropZone = (DropZone) this.ourRobot.getContext().getObjects(DropZone.class).get(0);
		objects.add(new ObjectInformation(dropZone));

		// Add rooms
		IndexedIterable<Object> allRooms = this.ourRobot.getContext().getObjects(BlocksRoom.class);
	...
	public List<Long> getAtBlock() {
		...
		for (Object object : this.ourRobot.getContext().getObjects(Block.class)) {
			Block b = (Block) object;
			if (this.ourRobot.canPickUp(b)) {
	...
	public String getRoom() {
		...
		if (this.ourRobotRoom == null) {
			return null;
		} else {
		return this.ourRobotRoom.getName(); }
	}
	...
	public Point2D getLocation() {
		return new Point2D.Double(this.ourRobotLocation.getX(), this.ourRobotLocation.getY());
	}
	...
	public List<String> getRooms() {
		...
		IndexedIterable<Object> objects = this.ourRobot.getContext().getObjects(Zone.class);
	...
	public double getSpeed() {
		return this.ourRobot.getSpeedMod();
	}
	...
	public List<String> getPlayers() {
		...
		for (String agt : agents) {
			try {
				...
				if (!entities.contains(this.ourRobot.getName())) {
	...
	public List<Long> getHolding() {
		Stack<Block> holding = this.ourRobot.getHolding();
	...
	public List<Long> getHoldingBlocks() {
		...
		List<Block> blockstack = new ArrayList<>(this.ourRobot.getHolding());
	...
	public Integer gripperCapacity() {
		return this.ourRobot.getGripperCapacity();
	}
	...
	public List<String> getOccupied() {
		...
		for (Object r : this.ourRobot.getContext().getObjects(Room.class)) {
			Room room = (Room) r;
	...
	public List<ArrayList<String>> getMessages() {
		...
		LOGGER.log(BotLog.BOTLOG, msg);
		for (int i =0; i < msg.size(); i++) {
			// Insert code that evaluates whether the message received is a good one?
		}
		return msg;
	}
	...
	public String getState() {
		return this.ourRobot.getState().toString().toLowerCase();
	}
	...
	public double getBatteryPercentage() {
		return this.ourRobot.getBattery().getPercentage();
	}
	...
	public boolean isColorBlind() {
		return this.ourRobot.getHandicapsList().contains("ColorBlind");
	}
	...
	public void goTo(double x, double y) {
		this.ourRobot.setTargetLocation(new NdPoint(x, y));
	}
	...
	public void goTo(long targetid) {
		...
		this.ourRobot.setTarget(target);
	}
	...
	public void pickUp(long blockId) {
		LOGGER.debug(String.format("%s is trying to pick up a block.", this.ourRobot.getName()));
		Block block = getBlock(blockId);

		LOGGER.debug(String.format("%s will pickup block %d.", this.ourRobot.getName(), block.getId()));
		if (!this.ourRobot.canPickUp(block)) {
			return;
		}
		this.ourRobot.pickUp(block);
	}
	...
	public void navigateObstacles() {
		LOGGER.debug(String.format("%s is trying to navigate the following obstacles: ", this.ourRobot.getName()));
		for (BoundedMoveableObject obj : this.ourRobot.getObstacles()) {
			LOGGER.debug(obj + " at " + obj.getBoundingBox());
		}

		if (this.ourRobot instanceof NavigatingRobot) {
			NavigatingRobot navbot = (NavigatingRobot) this.ourRobot;
			navbot.navigateObstacles();
		} else if (this.ourRobot instanceof AbstractRobotDecorator) {
			IRobot robotEarliestParent = this.ourRobot.getEarliestParent();
	...
	public void sendMessage(String receiver, String message) throws ActException {
		this.ourRobot.getAgentRecord().addSentMessage();

		// Translate the message into parameters
		Parameter[] parameters = new Parameter[2];
		try {
			parameters[0] = Translator.getInstance().translate2Parameter(this.ourRobot.getName())[0];
	...
	public List<EPartner> getEPartners() {
		...
		EPartner mine = this.ourRobot.getEPartner();
	...
	public void pickUpEPartner() {
		LOGGER.debug(String.format("%s is trying to pick up an e-partner.", this.ourRobot.getName()));

		EPartner nearest = getClosest(EPartner.class);
		if (nearest == null) {
			LOGGER.debug(String.format("%s can not pickup any e-partners.", this.ourRobot.getName()));
			return;
		}
		LOGGER.debug(String.format("%s will pickup e-partner %d.", this.ourRobot.getName(), nearest.getId()));
		this.ourRobot.pickUpEPartner(nearest);
	}

	/**
	 * Only available for the human: drops the e-Partner they are currently holding.
	 */
	@AsAction(name = "putDownEPartner")
	public void dropEPartner() {
		if (this.ourRobot.isHuman()) {
			this.ourRobot.dropEPartner();
		}
	}
	...
	public List<String> getBumped() {
		...
		if (this.ourRobot.isCollided() && !this.ourRobot.getObstacles().isEmpty()) {
			for (BoundedMoveableObject obj : this.ourRobot.getObstacles()) {
	...
	public IdAndBoolean getOldTargetUnreachable() {
		return new IdAndBoolean(this.ourRobot.getId(), this.ourRobot.isDestinationUnreachable());
	}
	...
	/**
	 * Reset the robot's location and should set it to its default spawn state. We
	 * try to have the bot drop all the blocks before it exists.
	 */
	protected void reset() {
		while (!this.ourRobot.getHolding().isEmpty()) {
			this.ourRobot.drop();
		}
		this.ourRobot.moveTo(this.spawnLocation.getX(), this.spawnLocation.getY());
	}
	...
	private <T extends BoundedMoveableObject> T getClosest(Class<T> type) {
		...
		Iterable<Object> allBlocks = this.ourRobot.getContext().getObjects(type);
		T nearest = null;
		double nearestDistance = Integer.MAX_VALUE;
		for (Object o : allBlocks) {
			@SuppressWarnings("unchecked")
			T aBlock = (T) o;
			double distance = this.ourRobot.distanceTo(aBlock);

			LOGGER.trace(
					String.format("%s is %f units away from %d.", this.ourRobot.getName(), distance, aBlock.getId()));
			if (this.ourRobot.canPickUp(aBlock)) {
				LOGGER.trace(
						String.format("%s can pick up %d checking distance.", this.ourRobot.getName(), aBlock.getId()));
