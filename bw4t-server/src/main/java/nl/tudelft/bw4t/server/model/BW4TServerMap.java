...
/**
 * A data structure containing both the map and the repast context that is
 * active within the current server.
 */
public class BW4TServerMap {
	...
	/**
	 * Create a new server map with the given {@link NewMap} and repast
	 * {@link Context}.
	 * 
	 * @param map
	 *            the map
	...
	/**
	 * Set the new {@link NewMap} for this server map, will notify attached
	 * listeners.
	 * 
	 * @param map
	 *            the new {@link NewMap}
	...
	/**
	 * Set a new repast {@link Context} for this server map, will notify attached
	 * listeners.
	 * 
	 * @param context
	...
	public ContinuousSpace<Object> getContinuousSpace() {
		if (this.continuousSpace == null && hasContext()) {
			this.continuousSpace = (ContinuousSpace<Object>) getContext().getProjection(MapLoader.PROJECTION_ID);
		}
		return this.continuousSpace;
	}
	...
	public Grid<Object> getGridSpace() {
		if (this.gridSpace == null && hasContext()) {
			this.gridSpace = (Grid<Object>) getContext().getProjection(MapLoader.GRID_PROJECTION_ID);
		}
	...
	public WeightedGraph<NdPoint, DefaultWeightedEdge> getZoneGraph() {
		if (this.graph == null && hasContext()) {
			this.graph = GraphHelper.generateZoneGraph(getObjectsFromContext(Zone.class));
		}
		return this.graph;
	}
	...
	/**
	 * Attach the given object so it get notified whenever there are changes to the
	 * {@link NewMap} or repast {@link Context}.
	 * 
	 * @param e
	...
	private void notifyMapChange() {
		for (BW4TServerMapListerner listener : this.listeners) {
