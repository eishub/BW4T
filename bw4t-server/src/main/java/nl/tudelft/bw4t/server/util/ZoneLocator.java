...
public final class ZoneLocator {
	...
	/**
	 * Find {@link Zone} containing given point. If there are multiple Zones at the
	 * given point, the result is one of these.
	 *
	 * @param x
	...
	public static Zone getZoneAt(double x, double y) {
		if (!BW4TEnvironment.getInstance().hasContext()) {
			return null;
		}
	...
	/**
	 * Find the {@link Zone} that is nearest to given point. If point is in a Zone,
	 * we always return that Zone. If the point is not in a zone, we return the
	 * nearest Zone.
	 *
	 * @param location
	 *            - The location to search for nearby Zones.
	 * @return The nearest Zone to the given point. {@code null} if there are no
	 *         Zones whatsoever.
	 */
	...
	/**
	 * Find the {@link Zone} that is nearest to given point. See also
