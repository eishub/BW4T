...
public class RoomLocatorTest {
	@Mock
	private static BlocksRoom BLOCKSROOM = mock(BlocksRoom.class);
	@Mock
	private static DropZone DROPZONE = mock(DropZone.class);
	@Mock
	private static BW4TEnvironment env = mock(BW4TEnvironment.class);

	/**
	 * A minor offset is done on certain elements in order to fulfill requirements
	 * of the definition of insideness.
	 */
	...
	@BeforeClass
	public static void setUp() throws NoSuchMethodException, SecurityException, IllegalAccessException,
		...
		when(env.hasContext()).thenReturn(true);
	 ...
	 *
	 * @param x
	 *            - The x coordinate to search with.
	 * @param y
	 *            - The y coordinate to search with.
	 * @param expectedOutAt
	 *            - The expected output of the getRoomAt function.
	 * @param expectedOutFor
	 *            - The expected output of the getRoomFor function.
	 */
	...
	public void getRoomAtTest() {
		Room result = RoomLocator.getRoomAt(this.x, this.y);
		assertTrue((result == null && this.expectedOutAt == null)
				|| (result != null && result.equals(this.expectedOutAt)));
	}

	/** We test the getRoomFor method for correctness. */
	@Test
	public void getRoomForTest() {
		Room result = RoomLocator.getRoomFor(this.x, this.y);
		assertTrue((result == null && this.expectedOutFor == null)
				|| (result != null && result.equals(this.expectedOutFor)));
	}
