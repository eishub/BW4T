...
public class EPartnerProcessor implements PerceptProcessor {
	...
	private static final Logger LOGGER = Logger
			.getLogger(EPartnerProcessor.class.getName());

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * nl.tudelft.bw4t.client.controller.percept.processors.PerceptProcessor
	...
	public void process(List<Parameter> parameters,
		...
		epartner = initializeEPartner(clientMapController, id, entityId,
				holderId, epartner, types);
	 ...
	 */
	private ViewEPartner initializeEPartner(
			ClientMapController clientMapController, long id, String entityId,
			long holderId, ViewEPartner epartner, ParameterList types) {
		if (epartner == null) {
			epartner = new ViewEPartner();
			epartner.setId(id);
			clientMapController.addEPartner(epartner);
		}
