...
public class ActionPopUpMenu {
    ...
    public static void buildPopUpMenu(BW4TClientGUI bw4tClientMapRenderer) {
        ...
        for (BlockColor color : bw4tClientInfo.environmentDatabase.getSequence()) {
            Shape colorBounds = new Rectangle2D.Double(startPosX,
                    VisualizerSettings.worldY * VisualizerSettings.scale, 20,
                    20);
            if (colorBounds.contains(new Point(bw4tClientInfo.selectedLocation[0],
                    bw4tClientInfo.selectedLocation[1]))) {
                MapOperations.buildPopUpMenuForGoalColor(color, bw4tClientMapRenderer);
                bw4tClientInfo.jPopupMenu.show(bw4tClientMapRenderer, bw4tClientInfo.selectedLocation[0],
                        bw4tClientInfo.selectedLocation[1]);
                return;
            }
            startPosX += 20;
        ...
        for (RoomInfo room : bw4tClientInfo.environmentDatabase.getRooms()) {
            Shape roomBoundaries = MapOperations.transformRectangle(new Rectangle2D.Double(
                    room.getX(), room.getY(), room.getWidth(), room.getHeight()));
            if (roomBoundaries.contains(new Point(bw4tClientInfo.selectedLocation[0],
                    bw4tClientInfo.selectedLocation[1]))) {
                // Check if pressing on a block
                for (Long boxID : bw4tClientInfo.environmentDatabase.getVisibleBlocks()) {
                    HashMap<Long, java.awt.geom.Point2D.Double> objectPositions = bw4tClientInfo.environmentDatabase
                            .getObjectPositions();
                    Shape boxBoundaries = MapOperations.transformRectangle(new Rectangle2D.Double(
                            objectPositions.get(boxID).getX(), objectPositions
                                    .get(boxID).getY(), Constants.BLOCK_SIZE,
                            Constants.BLOCK_SIZE));
                    if (boxBoundaries
                            .contains(new Point(bw4tClientInfo.selectedLocation[0],
                                    bw4tClientInfo.selectedLocation[1]))) {
        ...
        // Check for dropzone
        DropZoneInfo dropZone = bw4tClientInfo.environmentDatabase.getDropZone();
        Shape dropZoneBoundaries = MapOperations.transformRectangle(new Rectangle2D.Double(
                dropZone.getX(), dropZone.getY(), dropZone.getWidth(),
                dropZone.getHeight()));
        if (dropZoneBoundaries.contains(new Point(bw4tClientInfo.selectedLocation[0],
                bw4tClientInfo.selectedLocation[1]))) {
