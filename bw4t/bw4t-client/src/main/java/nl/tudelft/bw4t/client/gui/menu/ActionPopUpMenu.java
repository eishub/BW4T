...
public class ActionPopUpMenu {
    ...
    public static void buildPopUpMenu(BW4TClientGUI bw4tClientMapRenderer) {
        ...
        for (BlockColor color : bw4tClientInfo.getEnvironmentDatabase().getSequence()) {
            Shape colorBounds = new Rectangle2D.Double(startPosX, VisualizerSettings.worldY * VisualizerSettings.scale,
                    20, 20);
            if (colorBounds.contains(new Point(bw4tClientInfo.getSelectedLocation()[0], bw4tClientInfo
                    .getSelectedLocation()[1]))) {
                MapOperations.buildPopUpMenuForGoalColor(color, bw4tClientMapRenderer);
                bw4tClientInfo.getjPopupMenu().show(bw4tClientMapRenderer, bw4tClientInfo.getSelectedLocation()[0],
                        bw4tClientInfo.getSelectedLocation()[1]);
                return;
            }
            startPosX += 20;
        }
        ...
        for (RoomInfo room : bw4tClientInfo.getEnvironmentDatabase().getRooms()) {
            Shape roomBoundaries = MapOperations.transformRectangle(new Rectangle2D.Double(room.getX(), room.getY(),
                    room.getWidth(), room.getHeight()));
            if (roomBoundaries.contains(new Point(bw4tClientInfo.getSelectedLocation()[0], bw4tClientInfo
                    .getSelectedLocation()[1]))) {
                // Check if pressing on a block
                for (Long boxID : bw4tClientInfo.getEnvironmentDatabase().getVisibleBlocks()) {
                    ...
                    if (boxBoundaries.contains(new Point(bw4tClientInfo.getSelectedLocation()[0], bw4tClientInfo
                        ...
                        if (MapOperations.closeToBox(boxID, bw4tClientInfo)) {
                            RoomMenus.buildPopUpMenuForBeingAtBlock(boxID, room, bw4tClientMapRenderer);
                            bw4tClientInfo.getjPopupMenu().show(bw4tClientMapRenderer,
                                    bw4tClientInfo.getSelectedLocation()[0], bw4tClientInfo.getSelectedLocation()[1]);
                        } else {
                            RoomMenus.buildPopUpMenuForBlock(boxID, room, bw4tClientMapRenderer);
                            bw4tClientInfo.getjPopupMenu().show(bw4tClientMapRenderer,
                                    bw4tClientInfo.getSelectedLocation()[0], bw4tClientInfo.getSelectedLocation()[1]);
                        }
                        return;
        ...
        // Check for dropzone
        DropZoneInfo dropZone = bw4tClientInfo.getEnvironmentDatabase().getDropZone();
        ...
        if (dropZoneBoundaries.contains(new Point(bw4tClientInfo.getSelectedLocation()[0], bw4tClientInfo
                .getSelectedLocation()[1]))) {
