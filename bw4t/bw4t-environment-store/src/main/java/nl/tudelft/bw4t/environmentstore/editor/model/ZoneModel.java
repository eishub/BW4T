...
public class ZoneModel {
	...
	private void changeNeighbourDoor(int dir, int oppositdir) {
		if (neighbours[dir] != null
				...
				&& (neighbours[dir].hasDoor(oppositdir) || !neighbours[dir]
						.hasDoors())) {
	...
	/**
	 * If the neighbour is a corridor or charge zone, we can place a door on
	 * this side.
	 */
	...
	private boolean isStartZone(Zone zone) {
		return zone != null && zone.getName() != null
				&& zone.getName().startsWith("StartZone");
	}

	private boolean isDropZone(Zone zone) {
		return zone != null && zone.getName() != null
				&& zone.getName().startsWith(Zone.DROP_ZONE_NAME);
	}
	 ...
	 * 
	 * @param amount
	 *            the max amount of blocks in that zone
	 * @param validcolors
