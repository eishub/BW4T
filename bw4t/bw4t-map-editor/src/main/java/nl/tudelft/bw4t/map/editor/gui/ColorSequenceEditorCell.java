 ...
 */
public class ColorSequenceEditorCell extends JTextField {
     ...
     *            is initial sequence.
     */
    public ColorSequenceEditorCell(ColorSequence cols) {
        if (cols == null) {
    ...
    public ColorSequence getColors() {
        ...
        if (letters.length() > 10) {
            EnvironmentStore.showDialog("More than 10 letters, ignoring them");
        ...
        for (Character letter : letters.toUpperCase().toCharArray()) {
            try {
                ...
                EnvironmentStore.showDialog(e, e.getMessage()); // and continue the loop.
            }
        }
        return colors;
    }
